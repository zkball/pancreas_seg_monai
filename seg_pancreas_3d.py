...
class seg_pancreas_3d():

    def __init__(
        self,
    ) -> None:
        self.num_epoch  = 100 if mode=="2d" else 1000
        num_epoch = num_epoch if args.eval is False else 1
        
        self.batch_size = 4
    
    def define_batch_size(self, mode="3d"):
        if mode == "3d":
            batch_size = 4
        else:
            batch_size = 8
    ...
    def prepare_data(self, mode="3d", image_dir=None, mask_dir=None, file_format=None, only_eval=False, image_test_dir=None, negative_dir=None, batch_size=None):
    ...
    def prepare_model(self, mode, pretrain=None, device=None):
        if mode == "3d":
    ...
    def window_adjust(self, data, w_width=250, w_min=-75, w_max=175):
    ...
    def main(self, args):
        ...
        model = self.prepare_model(mode=mode, pretrain=args.pretrain, device=device)
        loss_function = self.prepare_losses(mode=mode)
        ...
        for epoch in range(num_epoch):
            ...
            if args.eval or (not args.eval and (epoch + 1) % val_interval == 0):
                ...
                with torch.no_grad():
                    ...
                    ######### test - if have 
                    if best_metric>0.5 and metric == best_metric:
