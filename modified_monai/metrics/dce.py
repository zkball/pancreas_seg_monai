...
class DCE(CumulativeIterationMetric):
    """
...
def compute_iou(
    ...
    if ignore_empty:
        return torch.where(y_o > 0, 2* (intersection) / union, torch.tensor(float("nan"), device=y_o.device))
    return torch.where(union > 0, 2* (intersection) / union, torch.tensor(1.0, device=y_o.device))
