...
class PreNorm(nn.Module):
    def __init__(self, dim, fn, groups=4):
        super().__init__()
        self.fn = fn
...
class Block(nn.Module):
    def __init__(self, dim, dim_out, groups = 4, use_bayar=False):
        super().__init__()
        conv = nn.Conv2d 
        if use_bayar:
            self.proj = BayarConv(dim, dim_out, groups=2)
        else:
            self.proj = conv(dim, dim_out, 5, padding = 2) 
...
class Unet(nn.Module):
    def __init__(
        ...
        if self.use_bayar:
            self.BayarConv2D = BayarConv(input_channels, self.use_bayar, groups=1)
        ...
        self.mid_attn = PreNorm(mid_dim, nn.Identity()),
...
if __name__ == '__main__':
    model = Unet(use_bayar=3, use_fft=True, use_SRM=False, use_hierarchical_segment=[1],
                                                                use_classification=[], 
                                                                use_hierarchical_class=[1],
                                                                use_normal_output=[]).cuda()
    input = torch.ones((3,3,128,128)).cuda()
    middle_feats, hier_class_output, hier_seg_output, x_cls, outs = model(input, torch.zeros((1)).cuda())
